...
uses
  ...
  Vcl.ComCtrls, ACBrSocket, ACBrNCMs,  Vcl.Menus,
  IdBaseComponent, IdComponent,uni,
  IdTCPConnection, IdTCPClient, IdHTTP, Vcl.DBGrids,Vcl.Imaging.pngimage,
   MemDS,
  DBAccess,  Vcl.Grids, JvExControls, JvDBLookup;
...
type
  TFrmCadProduto = class(TForm)
    ...
    DBLookupComboboxEh3: TJvDBLookupCombo;
    DBLookupComboboxEh1: TJvDBLookupCombo;
    DBLookupComboboxEh4: TJvDBLookupCombo;
    DBEdit20: TDBEdit;
    DBEdit25: TDBEdit;
    DBEdit8: TDBEdit;
    DBImage1: TDBImage;
    DBEdit32: TDBEdit;
    DBEdit33: TDBEdit;
    DBLookupComboboxEh2: TJvDBLookupCombo;
    btnMarca: TBitBtn;
    DBEdit40: TDBEdit;
    DBEdit48: TDBEdit;
    Panel4: TPanel;
  ...
  try
    try
      ...
      if not dados.EhSoNumero(sGTIN) then
        begin
          Application.MessageBox(PChar('Digite Um Código de '+
          ...
          if (Trim(Response.DataString) <> '') then
            begin
              ...
              if not (qryProdutos.State in dsEditModes) then
                qryProdutos.Edit;
              qryProdutosDESCRICAO.AsString := Copy(RESTDWClientSQL.FieldByName('Nome').AsString, 1, 100);
              qryProdutosNCM.AsString :=  RESTDWClientSQL.FieldByName('Ncm').AsString;
              qryProdutosCEST.AsString :=  RESTDWClientSQL.FieldByName('Cest_Codigo').AsString;
              url := Format('http://www.eanpictures.com.br:9000/api/gtin/%s', [sGTIN]);
    ...
    on E: Exception do
      raise Exception.Create('Falha ao consultar dados do produto.'+
...
//update 5
procedure TFrmCadProduto.GetImageByUrl(URL: string; sGTIN: string; out wFile: string);
var
  sDir: string;
  sFile: string;
